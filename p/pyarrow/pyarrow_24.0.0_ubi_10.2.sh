...
PACKAGE_VERSION=${1:-apache-arrow-25.0.0}
...
python3.14 -m pip install "${WHEEL}" --no-deps
SITE_PACKAGES=$(python3.14 -m pip show pyarrow | awk -F': ' '/^Location:/ {print $2}')

export LD_LIBRARY_PATH="${GRPC_PREFIX}/lib:${PROTOBUF_PREFIX}/lib64:${ORC_PREFIX}/lib:${THRIFT_PREFIX}/lib:${BOOST_PREFIX}/lib:${RE2_PREFIX}/lib:${CA_PREFIX}/lib:${XS_PREFIX}/lib:${UTF_PREFIX}/lib64:${S_PREFIX}/lib64:${ZSTD_HOME}/lib:/usr/lib64:/usr/lib:$SITE_PACKAGES/pyarrow:${LD_LIBRARY_PATH}"

python3.14 -m pip install -r $SCRIPT_DIR/arrow/python/requirements-test.txt "pytest<9"

PYARROW_LOCATION=$(python3.14 -c "import os; import pyarrow; print(os.path.dirname(pyarrow.__file__))")
...
if ! python3.14 -m pytest -k "not test_foreign_buffer and not test_get_include and not pandas" $PYARROW_LOCATION -vv ; then
