...
pip install  packaging pathspec pluggy trove-classifiers wheel build hatchling joblib threadpoolctl scikit-build-core

# Install dependencies required for running the XGBoost Python test suite
pip install pytest hypothesis pandas matplotlib pyarrow dask modin shap  pyspark modin[ray] modin[dask] modin[unidist] graphviz
...
python3.14 -m pip wheel -w ${OUTPUT_FOLDER} -v . --no-build-isolation --no-deps

popd

# Install locally built wheel for validation
if ! (pip install ${OUTPUT_FOLDER}/xgboost-*.whl  --no-deps); then
