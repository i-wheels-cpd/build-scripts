...
GITHUB_BUILD_SCRIPT_BASE_OWNER = "i-wheels-cpd"
HOME = os.getcwd()
...
#     eof = os.popen('file ' + script_path).read()
#     if 'crlf' in eof.lower():
#         raise EOFError(f"Build script {file_name} contains Windows line endings (CRLF). Please use Linux (LF).")
...
#                         package_data["distro_name"] = distro_data[0].lower()
#                         package_data["distro_version"] = distro_data[-1]
#                     elif key in line:
#                         matched_keys.append(key)
#                         package_data[key_checks[key]] = line.split(':', 1)[-1].strip()
...
#         else:
#             print("Basic Validation Checks Failed!")
#             print("Required keys:", ",".join(key_checks.keys()))
#             print("Found keys:", ",".join(matched_keys))
#             print("Missing:", ",".join(set(key_checks.keys()) - set(matched_keys)))
#             raise ValueError(f"Basic Validation Checks Failed for {file_name}!")
#     else:
#         raise ValueError("Build script not found.")
...
# def build_non_root_custom_docker_image(distro_version):
#     global image_name
#     base_image = f"registry.access.redhat.com/ubi9/ubi:{distro_version}"
#     print(f"Building non-root custom image from: {base_image}")
...
#     try:
#         script_path = os.path.join(HOME, file_name)
#         required_fields = ['package_name', 'github_url', 'version', 'default_branch', 'build_script', 'package_dir', 'maintainer', 'use_non_root_user']
#         data = json.load(open(script_path, 'r'))

#         for field in required_fields:
#             if field not in data:
#                 raise ValueError(f"No `{field}` field available in {file_name}.")

#         if str(data['github_url']).endswith('/'):
#             raise Exception(f"Invalid GitHub URL in {file_name} (ends with '/')")

#         with open(script_path, 'r') as f:
#             for line in f:
#                 if line.isspace():
#                     raise Exception(f"Empty line found in {file_name}")

#         global use_non_root
#         use_non_root = str(data['use_non_root_user']).lower()
#         print("Non-root user:", use_non_root)

#         print("build_info.json validated")
#         return True
#     except Exception as e:
#         print(str(e))
#         print(f" Failed to validate build_info file at {file_name}")
#         raise e

# def trigger_build_validation_travis(pr_number):
#     global image_name

#     url = f"https://api.github.com/repos/{GITHUB_BUILD_SCRIPT_BASE_OWNER}/{GITHUB_BUILD_SCRIPT_BASE_REPO}/pulls/{pr_number}/files"
#     validated_file_list = []
...
#         status = f.get('status', "")
#         if not file_name or status == "removed":
#             continue

#         if file_name.endswith('.sh') and "dockerfile" not in file_name.lower():
#             trigger_basic_validation_checks(file_name)

#             # Set container image based on "# Tested on" field
#             distro = package_data.get("distro_name", "")
#             version = package_data.get("distro_version", "")
#             if distro == "ubi":
#                 image_name = f"registry.access.redhat.com/ubi9/ubi:{version}"
#             else:
#                 raise Exception(f"Unsupported distro: {distro}")

#             if use_non_root == "true":
#                 build_non_root_custom_docker_image(version)

#             if package_data.get('travis_check', '').lower() == "true":
#                 trigger_script_validation_checks(file_name)
#             else:
#                 print(f"Skipping build script validation for {file_name} (Travis-Check: False)")

#             validated_file_list.append(file_name)

#         elif file_name.endswith('build_info.json'):
#             validate_build_info_file(file_name)
#             validated_file_list.append(file_name)
...
#     else:
#         print("Validated the following files:")
#         print(*validated_file_list, sep="\n")
