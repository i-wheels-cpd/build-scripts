...
if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
    ...
    response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/gzip" -T $1 "https://s3.us.cloud-object-storage.appdomain.cloud/ose-x86-toolci-bucket-production/$PACKAGE_NAME/$VERSION/$1")
