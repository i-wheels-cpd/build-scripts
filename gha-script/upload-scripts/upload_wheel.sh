...
token_request=$(curl -X POST https://iam.cloud.ibm.com/identity/token \
  ...
  -d "grant_type=urn%3Aibm%3Aparams%3Aoauth%3Agrant-type%3Aapikey&apikey=$GHA_CURRENCY_SERVICE_ID_X86_API_KEY")
...
if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
    ...
    response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/octet-stream" -T $1 "https://s3.us.cloud-object-storage.appdomain.cloud/ose-x86-artifacts-production/$PACKAGE_NAME/$VERSION/$1")
