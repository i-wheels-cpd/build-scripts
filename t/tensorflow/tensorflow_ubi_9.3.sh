...
yum install -y git make cmake zip tar wget python3.12 python3.12-devel python3.12-pip gcc-toolset-13 gcc-toolset-13-gcc-c++ gcc-toolset-13-gcc zlib-devel libjpeg-devel openssl openssl-devel freetype-devel pkgconfig ninja-build java-11-openjdk java-11-openjdk-devel java-11-openjdk-headless
...
export TF_CUDA_COMPUTE_CAPABILITIES="sm_60,sm_61,sm_70,sm_75,sm_80,sm_86,sm_90,compute_80,compute_86,compute_90"
export CUDA_NVCC=1
export HERMETIC_CUDA_COMPUTE_CAPABILITIES="sm_60,sm_61,sm_70,sm_75,sm_80,sm_86,sm_90,compute_80,compute_86,compute_90"
...
cat > nvidia_components_configure.bazelrc << EOF
build --config=cuda
build --action_env TF_CUDA_VERSION="12.6.3"
build --action_env TF_CUDNN_VERSION="9" #First digit only
build --action_env TF_NCCL_VERSION="2.21.5"
...
#Build package
if ! bazel --bazelrc=$SRC_DIR/tensorflow/tensorflow.bazelrc build --local_cpu_resources=HOST_CPUS*0.50 --local_ram_resources=HOST_RAM*0.50 --jobs=$BAZEL_JOBS   --config=cuda_wheel ${BUILD_TARGET} ; then
