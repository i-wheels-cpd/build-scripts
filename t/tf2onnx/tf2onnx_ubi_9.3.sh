...
yum install -y libffi-devel openssl-devel sqlite-devel zip rsync python3-devel
...
# Set JAVA_HOME using the expected x86_64 path
export JAVA_HOME=/usr/lib/jvm/java-11-openjdk-11.0.25.0.9-3.el9.x86_64

# Or set JAVA_HOME dynamically (for x86_64)
export JAVA_HOME=/usr/lib/jvm/$(ls /usr/lib/jvm/ | grep -P '^(?=.*java-)(?=.*x86_64)')
...
export LD_LIBRARY_PATH="$(pwd)/protobuf/build:$LD_LIBRARY_PATH"
...
echo "cpu_opt_arch=${cpu_opt_arch:-native}"
echo "cpu_opt_tune=${cpu_opt_tune:-native}"
...
# Set defaults for CPU architecture options
cpu_opt_arch=${cpu_opt_arch:-native}
cpu_opt_tune=${cpu_opt_tune:-native}

# Now build compiler options using the correct values
CPU_ARCH_OPTION="build:opt --copt=-mcpu=${cpu_opt_arch}"
CPU_ARCH_HOST_OPTION="build:opt --host_copt=-mcpu=${cpu_opt_arch}"
CPU_TUNE_OPTION="build:opt --copt=-mtune=${cpu_opt_tune}"
CPU_TUNE_HOST_OPTION="build:opt --host_copt=-mtune=${cpu_opt_tune}"
