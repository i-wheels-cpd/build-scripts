...
git clone -b v1.18.0 https://github.com/scipy/scipy
cd scipy
git submodule update --init
...
SCIPY_WHEEL=$(find "${CURRENT_DIR}/dist" -name "scipy-*.whl" | head -1)
if [ -z "$SCIPY_WHEEL" ]; then
    echo "ERROR: scipy wheel not found after build"
    exit 1
fi
python3.14 -m pip install "$SCIPY_WHEEL"

cd "${CURRENT_DIR}"
echo "--------------------scipy installed-------------------------------"

# -----------------------------------------------------------------------------
# Build scikit-learn 1.9.0 from source
# -----------------------------------------------------------------------------
echo "--------------------------------- scikit-learn Installing ---------------------------------"

python3.14 -m pip install joblib threadpoolctl
...
# Build wheel
python3.14 -m pip wheel -vv --no-build-isolation --no-deps . -w "${CURRENT_DIR}/dist/"

SK_WHEEL=$(find "${CURRENT_DIR}/dist" -name "scikit_learn-*.whl" | head -1)
if [ -z "$SK_WHEEL" ]; then
    echo "ERROR: scikit-learn wheel not found after build"
    exit 1
fi

# Install scikit-learn
if ! python3.14 -m pip install "$SK_WHEEL" ; then
...
# Test using pytest
# Set below flag as suggested in GitHub forums to resolve ImportPathMismatchError
export PY_IGNORE_IMPORTMISMATCH=1
cd "${CURRENT_DIR}"

if ! pytest --pyargs sklearn.tests.test_random_projection; then
